...
const SW_VERSION = 'v74'; // v73 -> v74: Estadísticas ahora incluye Censos y Ayudas Humanitarias, que antes no aparecían en ningún lado del módulo (datos.js pasó de leer 2 colecciones a 4: emergencias/inspecciones/censos/ayudas_humanitarias). Se agregaron dos pestañas nuevas ("Censos" y "Ayudas Humanitarias") con sus propias tarjetas KPI y gráficas (barrio/vereda, infraestructura afectada, tipo de ocupante, tipo de kit entregado, censados vs. pendientes), y la Vista General pasó de 4 a 6 tarjetas resumen. Además, en el Panel General, las tarjetas "Censos" y "Ayudas" — que llevaban marcadas como "sin datos" desde que existen, esperando a que estos módulos se construyeran — ya muestran cifras reales de Firestore (censos: total de registros; ayudas: total de kits entregados, sumando cantidadEntregada de cada entrega).
